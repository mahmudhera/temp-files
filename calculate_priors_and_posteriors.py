...
def expectation_maximization2(data_points_raw, init_mean, max_count, max_k):
    ...
    while True:
        ...
        estimated_averaged_lambda = 1.0 * np.dot( np.array(lambdas), np.array(ownership_factors) ) / sum(ownership_factors)
    ...
    while True:
        ...
        for j in data_points.keys():
            ...
            for ii in range(max_count):
                total += probabilities[ii][j] * priors[ii]
            denominators[j] = total
...
def determine_priors_posteriors(histo_data, max_priors, savgol_filter_window):
    ...
    estimated_kmer_coverage, priors, poissons, probabilities = expectation_maximization2(histo_data, init_coverage, max_priors, max_k)
